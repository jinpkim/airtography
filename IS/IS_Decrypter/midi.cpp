...
{ 
	...
	while(!input.eof())
	{
		...
		getline(input, str);
		//input >> value;
		if (str.find("NoteOnEvent") != string::npos || str.find("NoteOffEvent") != string::npos)
		{
			int pos1 = str.find("Note=\"", 0);
			int pos2 = str.find("\" Velocity=", 0);
			value = str.substr(pos1, pos2-pos1);
			if (str.find(note) != string::npos) 
			{
				string notevalue ("");
...
int* midi::ConvertToIntA()
{
	int * ret = new int[this->GetInputSize()];
	for (int i = 0; i < decode.size(); ++i)
	{
		ret[i] = atoi(decode[i].c_str());
	}
